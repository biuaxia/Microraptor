...
import cn.biauxia.code.microraptor.v1.core.common.web.ApiResult;
import cn.biauxia.code.microraptor.v1.core.common.web.RequestDetail;
import cn.biauxia.code.microraptor.v1.core.exception.*;
import cn.biauxia.code.microraptor.v1.utils.RequestDetailThreadLocal;
...
public class GlobalExceptionConf {
    ...
    public ApiResult<List<String>> handleMethodArgumentNotValidExceptionHandler(MethodArgumentNotValidException ex) {
        ...
        log.error(getApiCodeString(ApiCode.PARAMETER_EXCEPTION) + ":" + JSONUtil.toJsonStr(list));
    ...
    @ResponseStatus(HttpStatus.OK)
    public ApiResult<Boolean> springBootPlusExceptionHandler(IException exception) {
        printRequestDetail();
        log.error("IException:", exception);
        int errorCode;
        if (exception instanceof BusinessException) {
            errorCode = ApiCode.BUSINESS_EXCEPTION.getCode();
        } else if (exception instanceof MapperException) {
